...
def discover_onvif_devices():
    ...
    # 반드시 namespace와 localname을 분리해서 전달해야 함
    scope = QName("http://www.onvif.org/ver10/network/wsdl", "NetworkVideoTransmitter")

    # 혹은 모든 장치를 검색하고 싶다면: services = wsd.searchServices()
    services = wsd.searchServices(types=scope)

    discovered = []
    for service in services:
        xaddrs = service.getXAddrs()  # 예: ['http://192.168.0.10:80/onvif/device_service', ...]
        epr = service.getEPR()        # 장치 고유 식별자
